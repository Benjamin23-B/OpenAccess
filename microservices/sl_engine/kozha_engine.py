"""
Kozha translation engine.

Faithful port of the real zhan-a/Kozha server translation pipeline
(``server/server.py``) into a self-contained module, so the Deaf Bridge can
use the genuine Kozha NLP: spaCy tokenisation/lemmatisation/POS + per-sign-
language grammar reordering producing a normalized *gloss* string.

The gloss string is consumed client-side, where the gloss->SiGML mapping
happens by looking each gloss up in the shipped ``*.sigml`` sign databases
(via ``kozhaClient`` in the Next.js app). This mirrors Kozha's real split.

Only the English spaCy model (``en_core_web_sm``) is required: BSL and ISL
both gloss from English. ``get_nlp`` keeps Kozha's fallback-to-English
behaviour for any language whose model is not installed.
"""
...
try:
    ...
    print(
        "[ERROR] English spaCy model 'en_core_web_sm' not installed. Run: "
...
PRONOUNS = {
    ...
    "de": {
        "keep": {"du", "wir", "ihr", "uns", "nicht", "nichts", "und", "oder", "aber", "wenn", "weil"},
        "normalize": {
    ...
    "fr": {
        "keep": {"tu", "nous", "vous", "moi", "toi", "mon", "ma", "mes", "ton", "ta", "tes",
                 "son", "sa", "ses", "notre", "votre", "leur", "pas", "très", "plus", "moins",
                 "et", "ou", "mais"},
        "normalize": {
            "je": "moi", "j'": "moi",
            "il": "il", "lui": "il",
    ...
    "pl": {
        "keep": {"ja", "ty", "my", "wy", "oni", "one", "on", "mój", "twój", "nasz", "wasz",
                 "jego", "jej", "ich", "nie", "nic", "nikt"},
        "normalize": {
    ...
    "nl": {
        "keep": {"jij", "wij", "hij", "ons", "u", "mijn", "jouw", "dit", "deze",
...
def process_sentence(doc_or_span, stopwords: set, time_words: set, pronouns: dict,
                     sign_language: str, abbr_spans=None,
...
def plan_from_text(
    ...
    """Return the real Kozha plan shape: a normalized *gloss* string in ``final``.

    The client is responsible for mapping each gloss to its SiGML entry in
    the sign-language database (see ``kozhaClient`` in the frontend).
    """
    text = (text or "").strip()
    if not text:
...
def signature_note() -> str:
    return (
        "Kozha translation engine (server/server.py, zhan-a/Kozha) — "
        "faithfully ported for BSL/ISL. English spaCy model: en_core_web_sm."
    )